...
class Setup extends Command
{
    ...
    public function handle()
    {
        ...
        $auth_mode = $this->choice('Authorization Mode', [
            ...
            'custom' => 'The access token is generated at current application via callback page (App/Http/Controllers/Auth/SallaAuthController.php)',
        ], 'easy', 3);
        if (! empty($auth_mode)) {
        ...
        $db_connection = $this->choice('Database Connection', [
            'sqlite' => 'SQLite (file-based, no credentials needed)',
            'mysql' => 'MySQL',
        ], 'sqlite');
        $this->writeNewEnvironmentFileWith($db_connection, 'DB_CONNECTION');
